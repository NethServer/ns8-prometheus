...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-prometheus; then
    ...
    buildah from --name nodebuilder-prometheus -v "${PWD}:/usr/src:Z" docker.io/library/node:24.13.1-alpine
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=quay.io/prometheus/prometheus:v3.5.1" \
    "${container}"
# Commit the image

...
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-prometheus; then
    ...
    buildah from --name nodebuilder-prometheus -v "${PWD}:/usr/src:Z" docker.io/library/node:24.15.0-alpine
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=quay.io/prometheus/prometheus:v3.11.3" \
    "${container}"
# Commit the image
